...
def test_get_nodes(mock_kube_client):
    mock_kube_client.list_nodes.return_value = {
        "success": True,
        "data": [
            {"name": "node-1", "status": "Ready", "version": "v1.28.2", "os": "linux", "cpu": "4", "memory": "8Gi"}
        ],
    }
...
def test_scale_deployment(mock_kube_client):
    mock_kube_client.scale_deployment.return_value = {"success": True, "message": "Scaled to 3 replicas"}
