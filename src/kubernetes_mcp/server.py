...
async def k8s_pod_list(namespace: str = "default", ctx: Context = None) -> str:
    ...
    for p in pods:
        badge = (
            ...
            if p["status"] in ("Failed", "CrashLoopBackOff", "Error", "ImagePullBackOff")
            else "🟡"
        )
...
async def k8s_pod_logs(
    namespace: str, pod_name: str, container_name: str | None = None, tail_lines: int = 100, ctx: Context = None
) -> str:
...
async def minikube_status(ctx: Context = None) -> str:
    ...
    lines = ["### Minikube Local Cluster Diagnostics\n"]
...
def main():
    ...
    web_thread = threading.Thread(
        target=lambda: uvicorn.run(web_app, host=WEB_HOST, port=WEB_PORT, log_level="warning"), daemon=True
    )
    web_thread.start()
